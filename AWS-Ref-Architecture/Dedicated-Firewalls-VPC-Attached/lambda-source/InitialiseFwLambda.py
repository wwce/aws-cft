...
def updateTGWFirewall(vpc_summary_route, fw_trust_ip, fw_untrust_ip, api_key, trustAZ_subnet_cidr, fw_untrust_int):
    ...
    # VPC Summary route is a comma seperated strig
    for route in vpc_summary_route.split(','):
        logger.info('Adding route {} to fw with gatweway {}'.format(route, trustAZ_subnet_gw))
...
def lambda_handler(event, context):
    ...
    fw3_untrust_ip = os.environ['fw3UntrustIp']
    fw4_untrust_ip = os.environ['fw4UntrustIp']
    ...
    fwdata = [{
        'name': 'fw1',
        'fw_trust_ip': fw1_trust_ip,
        'fw_untrust_ip': fw1_untrust_ip
    },
        {
            'name': 'fw2',
            'fw_trust_ip': fw2_trust_ip,
            'fw_untrust_ip': fw2_untrust_ip
        },
        {
            'name': 'fw3',
            'fw_trust_ip': fw3_trust_ip,
            'fw_untrust_ip': fw3_untrust_ip

        },
        {
            'name': 'fw4',
            'fw_trust_ip': fw4_trust_ip,
            'fw_untrust_ip': fw4_untrust_ip
        }]

    for fw_ in fwdata:
        logger.info('Processing fw data {} type(fw_)'.format(fw_))
        updateTGWFirewall(vpc_summary_route, fw_['fw_trust_ip'], fw_['fw_untrust_ip'], api_key, trustAZ1_subnet_cidr,
                          fw_untrust_int)
        logger.info('Updating firewall {}'.format(fw_['name']))
        panCommit(fw_['fw_trust_ip'], api_key, message="Updated route table and address object")
