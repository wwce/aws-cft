...
def create_tgw_vpn(fwUntrustPubIP, pa_asn, Region, cgw1Tag, table_name, tgwId, bucketName, tag, instanceId):
    ...
    if cgwId and N1T1 and N1T2:
        vpnId = create_vpn_connection_upload_to_s3(Region, tgwId, cgwId, N1T1, N1T2, tag, bucketName,
                                                   assumeRoleArn=None)
...
def lambda_handler(event, context):
    ...
    """
    Region = os.environ['Region']
    logger.info('region is {}'.format(Region))
    lambda_bucket_name = event.get('lambda_bucket_name')
    fw1instanceId = event["EC2InstanceId"]
    cgw1Tag = event["EC2InstanceId"]
    tag1 = event["EC2InstanceId"]
    ...
    fw_untrust_pub_ip = event.get('fwUntrustPubIP')
    ...
    data = {}
    fw1_vpnId, fw1_cgwId = create_tgw_vpn(fw_untrust_pub_ip, pa_asn, Region, cgw1Tag, table_name, tgwId,
                                          lambda_bucket_name, tag1, instanceId)
    # Delay creation to allow Dynamodb updates and VPN to stabilise
    logger.info('created fw1 vpn with vpnId {}'.format(fw1_vpnId))
    time.sleep(20)

    # Delay creation to allow Dynamodb updates and VPN to stabilise
    logger.info('created fw vpn with vpnId {}'.format(fw1_vpnId))

    if fw1_vpnId and fw1_cgwId:
        event.update({'Action': 'config_aws_success',
                      'fw1_vpnId': fw1_vpnId,
                      'fw1_cgwId': fw1_cgwId
                      })
        return event
    else:
        event.update({'Action': 'config_aws_failed'})
    return event
