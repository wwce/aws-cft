...
def get_tgw_vpn_attachmentid(resource_id):
    try:
        ...
        logger.info('Got response {} looking for vpn attachmentid from resourceid {}'.format(response, resource_id))
        res = response['TransitGatewayAttachments'][0].get('TransitGatewayAttachmentId')
        return res
    except Exception as e:
        print('{}'.format(e))
        return False
...
def config_gw(fwUntrustPubIP, gwMgmtPubIp, fwUntrustPrivIP, fwUntrustSubnet, pa_asn, Region, cgw1Tag, table_name, tgwId,
    ...
    while True:
        ...
        if err == 'cmd_error':
            ...
            break
    pa_dmz_priv_ip = fwUntrustPrivIP

    fwUntrustSubnetCidr = find_subnet_by_id(fwUntrustSubnet)['CidrBlock']
    # Get the gateway IP for the trust subnet
    fwUntrustSubnetGw = get_gw_ip(fwUntrustSubnetCidr)
